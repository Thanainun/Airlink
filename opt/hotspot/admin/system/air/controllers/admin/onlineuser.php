...
Class Onlineuser extends MY_Admin
{
	function __construct()
	{
		...
		$this->load->model(array('onlineusermodel','jsonmodel'));
	...
	function disconnect()
	{
		...
		$gip = $this->onlineusermodel->getNasip($user);

		$rep['rep'] = TRUE;
		$rep['msg'] = freeradius_disconnectuser($user , $this->config->item('radiuscommand'),$gip->ip.':'.$this->config->item('coaport'), $this->config->item('radiussecret'));
