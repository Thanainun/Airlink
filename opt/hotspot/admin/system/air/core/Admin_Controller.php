...
class MY_Admin extends Main_controller
{
	...
	function _add_header()
	{
		...
		$add_header = array('site-name'	=>	$conf_data['admin_header'],
							...
							'login-page'	=>	'<a target="_blank" href="http://'.$_SERVER['HTTP_HOST'].'/hotspot/index.php/gologin'.'" class="medium twitter button radius" style="text-decoration:none;"><i style="font-size:16px; padding-top:3px; padding-right:5px;" class="icon-desktop"></i> หน้าล็อกอิน</a> ',
	...
	function _add_head()
	{
		...
		$arr_service = array('FTP'=>'proftpd','SSH'=>'ssh','Mysql'=>'mysqld','HTTP'=>'apache2','Chilli'=>'chilli','Proxy'=>'squid','Radius'=>'freeradius');
		$output_ser = array();
		
		foreach ($arr_service AS $name=>$service)
		{
			$output_ser[$name] = (trim(shell_exec("ps -A | grep ".$service." | awk {'print \$4'}"))) ? '<span style="float:right;padding-right:10px;">ทำงาน</span>' : '<span style="float:right;padding-right:10px;">ไม่ทำงาน</span>';
		}
