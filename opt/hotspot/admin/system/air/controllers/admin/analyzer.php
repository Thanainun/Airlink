...
{ 
	function __construct()
	{
		...
		//load the models 
		$this->load->model(array('analyzermodel','usermodel','onlineusermodel'));
	...
	function index()
	{
		$data['pp'] = $this->analyzermodel->getAll();
		$data['cache_hit'] = sprintf("%.2f",$this->analyzermodel->getCachehit()->prec_all);
		$data['cache_hit_day'] = sprintf("%.2f",$this->analyzermodel->getHitperday()->perc_day);
		
		$this->template	->add_css('datatable/table_his.css?'._DATETIME)
						->add_js('datatable/jquery.dataTablesh.min.js?'._DATETIME)
						->add_js('datatable/analyzer.js?'._DATETIME)
						->write_view('left-content', 'admin/analyzer/analyzer_view',$data)
						->render();
	...
	function json()
	{
		...
		foreach($user_list->result() as $row)
		{
			...
			$jdata[] .= $count++;
			$jdata[] .= anchor('admin/analyzer/history/'.$row->username,$row->username,'title="ลายละเอียด"');
			$jdata[] .= $row->billingplan;
			$jdata[] .= $row->start_time;
			$jdata[] .= $row->valid_until;
	...
	function history() 
	{
		...
		$this->template ->add_css('datatable/table_his.css?'._DATETIME)
						->add_js($js,'embed')
						->add_js('datatable/jquery.dataTablesh.min.js?'._DATETIME)
						->add_js('datatable/users_history.js?'._DATETIME)
						->write_view('left-content', 'admin/analyzer/user_history', $data)
						->render();
	}
	...
	function json_history()
	{
		...
		{
			$que = $this->analyzermodel->getProxyhistory($value->acctstarttime,$value->acctstoptime,$value->framedipaddress,$this->uri->segment(4));
	...
	function receipts()
	{
		...
		&y_ticks=5,10,10&
		&x_labels=มกราคม,February,March,April,May,June,July,August,September,October,November,December&
		&y_min=0& 
		&bg_colour=#FDFDFD&
		&x_axis_colour=#909090&
